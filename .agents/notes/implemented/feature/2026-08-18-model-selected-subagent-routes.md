...
An enabled definition registers `list_subagent_models`. With no arguments the tool lists authorized registered providers; with `provider` it calls that adapter's advisory model catalog only after authorization; with `provider` and `model` it authorizes the exact route before resolving its reasoning efforts and default. Because the discovery name is global, enabled instances in one tool scope share a single reference-counted registration: the first instance's policy defines the shared definition, which survives until the last enabled instance is disposed. Shipped product compositions put `modelSelectionSettings: true` on the primary Agent-scoped `subagent` instance and register the default-off Host-owned `subagent-model-selection` setting with an explicit `enabled` switch and `allowedModels` list. The Plugins settings page stores both fields atomically. A new top-level Session snapshots the route policy as `subagent/model-selection-policy` when the setting is enabled, before any model request. A child Session inherits the live parent's policy, and a resumed Session uses its recorded event instead of current settings. Therefore a settings edit affects only subsequently composed top-level Sessions. The fixed discovery definition remains available without the optional LLM service, while discovery and selected-route calls fail until that service is present. Discovery lists the intersection of the live catalog and recorded policy, and the executor rejects explicit routes outside it.

Shipped `subagent_fork` instances read model-selection settings when `modelSelectionSettings` is configured. A fork that changes provider or model forfeits the inherited prefix's KV Cache reuse; the tool description warns the caller that the route change can require the new provider to prefill the inherited conversation history, and that recomputation can dominate the delegated task's cost. The discovery tool's reference-counted registration lets spawn and fork instances coexist in one preset group without either instance's disposal stranding the other's selection schema.
...
**Enable model-facing route selection on shipped fork tools.** Shipped: the tool description warns that changing provider or model can prevent provider-side reuse of the inherited conversation prefix; the model makes the recomputation trade-off at call time.
...
- Shipped fork tools expose model-facing route selection when configured; the tool description warns that changing provider or model can prevent provider-side reuse of the inherited conversation prefix.
