...
**B 批已落地并真实验证。** `~/.dsh/hooks/cadence-audit.py`（软链到 `local-tools/tools/dsh-cadence-audit`）通过 9 个单测；`hooks-claude-code.json` 在 `UserPromptSubmit` 与 `PostToolUse` 上登记了它。重启 `dsh web` 后（插件启动时只读一次配置），新会话按要求连跑九次 `bash`，第八次之后收到用户角色注入 `Cost check: 8 consecutive tool calls, ~43s, no user-facing outcome yet. Produce an outcome or report the blocker before the next tool call.`；会话日志里有成对的 `hook/invoked`/`hook/result`，`exitCode: 0`。一处修正：状态目录是 `$TMPDIR/dsh-cadence`，不是 `~/.dsh/cadence`——hook 在 bash 沙箱内执行，沙箱可写根只有 workspace、`/tmp`、`tmpdir()`（[roots.ts](../../../../packages/sandbox/sandbox/src/roots.ts)）；首次真实运行在 `~/.dsh` 上报了 `Operation not permitted`。`Stop` 待定项保持待定：goal 驱动器的轮末裁判已对 goal 会话生效，尚无证据表明存在 `Stop` hook 才能补的缺口。
