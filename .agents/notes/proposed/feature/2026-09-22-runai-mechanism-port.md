...
**Batch B shipped and verified live.** `~/.dsh/hooks/cadence-audit.py` (symlink into `local-tools/tools/dsh-cadence-audit`) passes 9 unit tests; `hooks-claude-code.json` registers it on `UserPromptSubmit` and `PostToolUse`. After a `dsh web` restart (the plugin reads its config once at startup), a fresh session asked for nine sequential `bash` calls received `Cost check: 8 consecutive tool calls, ~43s, no user-facing outcome yet. Produce an outcome or report the blocker before the next tool call.` as a user-role injection after the eighth call; the session log shows `hook/invoked`/`hook/result` pairs with `exitCode: 0`. One correction: the state directory is `$TMPDIR/dsh-cadence`, not `~/.dsh/cadence` — hooks run inside the bash sandbox, whose writable roots are the workspace, `/tmp`, and `tmpdir()` ([roots.ts](../../../../packages/sandbox/sandbox/src/roots.ts)); the first live run failed with `Operation not permitted` on `~/.dsh`. The `Stop` open point stays open: the goal driver's round-end judge already fires for goal sessions, and no evidence yet shows a gap the `Stop` hook would close.
