...
另外，Session 删除需要 `SessionPersistence` 删除原语和 `session.delete` 端点。该空白的设计随本 Note 定案；产品路径现已在[会话删除](../../implemented/feature/2026-08-18-session-delete.md)落地。

后续的 [Workspace 注册记录删除决策](../../implemented/feature/2026-07-27-workspace-registration-deletion.md)取代的仅是上述耦合关系：删除 Workspace 注册记录会保留相关 Session 及其日志，Session 删除仍是独立能力。因此，下文的级联设计并不是 Workspace GUI 的删除语义。
...
| `SessionPersistence.delete` 扩面 + 级联删编排 | `packages/session/*` | 既有 seam 新方法 | 已在[会话删除](../../implemented/feature/2026-08-18-session-delete.md)落地 |
| `workspace.*` / `session.delete` RPC、GUI 接线、boot 组装 | — | — | workspace RPC 已交付；session delete 已在[会话删除](../../implemented/feature/2026-08-18-session-delete.md)落地 |
...
依赖方向：`dsh-workspace` → `dsh-domain` → `dsh-storage` ← 两后端。`dsh-workspace` 另依赖 `ctx.sessionPersistence` 的只读面（attach 的 cwd 校验读 session header；服务缺席时 attach 直接拒绝——无法校验即不写账）。session 删除相关的活动会话拆卸已在[会话删除](../../implemented/feature/2026-08-18-session-delete.md)落地。
...
- **Session 删除是独立能力。** 后续的 [Workspace 注册记录删除决策](../../implemented/feature/2026-07-27-workspace-registration-deletion.md)已将 `ctx.workspaceRegistry.delete(id)` 作为仅删除元数据、保留 Session 与日志的操作交付。递归删除 Session 已在[会话删除](../../implemented/feature/2026-08-18-session-delete.md)落地。
...
| session delete 约定（现已并入 `runPersistenceContract`；[会话删除](../../implemented/feature/2026-08-18-session-delete.md)） | 未知 id、已删 id 复用、未物化 intent、与在途 append 串行、deleted 事件 | jsonl、sqlite |
...
| 不做 | 触发条件 | 返工点 | 预埋 |
| --- | --- | --- | --- |
| Session 删除（`SessionPersistence.delete`、deleted 事件、递归删除、运行中检查） | 已在[会话删除](../../implemented/feature/2026-08-18-session-delete.md)落地 | 与 Workspace 注册记录删除保持独立 | 上文编排规则仍是基础；Workspace 删除仍会保留 Session 与日志 |
...
| Session 删除 RPC／GUI | 已在[会话删除](../../implemented/feature/2026-08-18-session-delete.md)落地 | 与 Workspace 注册记录删除保持独立 | Workspace RPC／GUI 仍是只删注册记录的独立路径 |
