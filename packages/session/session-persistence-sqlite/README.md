...
`delete(id)` removes the `sessions` row and cascades events. An unknown id rejects; an un-materialized create intent is cancelled without a row write.
...
- **No background historical compaction** — normal appends are insert-only; `delete(id)` removes one session rather than compacting retained history.
