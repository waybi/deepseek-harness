...
- **Delete.** `delete(id)` unlinks the session directory. An unknown id rejects; an un-materialized create intent is cancelled without touching disk.
...
- **Abandoned logs stay until `delete` or external removal** — `delete(id)` unlinks one session directory; there is no bulk or background cleanup.
