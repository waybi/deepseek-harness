...
| `delete(id): Promise<void>` | 永久删除一份会话的已存储日志。与同 id 的在途 append 串行。未知 id 拒绝。未实体化的 create 意图会被取消并成功返回。删除后该 id 对后续所有操作都视为未知。发出 `session-persistence/deleted`。 |
...
| `deleteStored(id)` | 永久移除一份已存储会话。移除了实体化工件时返回 true，不存在时返回 false。未知 id 拒绝由协调器负责。 |
