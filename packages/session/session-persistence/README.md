...
| `delete(id): Promise<void>` | Permanently remove one session's stored log. Serialized with in-flight appends on the same id. An unknown id rejects. An un-materialized create intent is cancelled and resolves. After deletion the id is unknown to every later operation. Emits `session-persistence/deleted`. |
...
| `deleteStored(id)` | Permanently remove one stored session. Returns true when a materialized artifact was removed, false when none existed. Unknown-id rejection belongs to the coordinator. |
