...
`session.delete` 永久删除一份会话日志。GUI 始终发送 `recursive: true`，同时删除所有通过 `parentSession` 到达的后代。非递归请求若仍有后代，则返回 `session-has-descendants`。未知 id 返回 `session-not-found`。本宿主无法释放的活动会话返回 `agent-busy`。网关先 dispose 它持有的 AgentHandle，再调用 `SessionPersistence.delete`；工作区注册表会从记账和归档集合中忘掉该 id。
